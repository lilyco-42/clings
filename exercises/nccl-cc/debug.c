...
static const char *tk_names[] = {
    "NUM", "IDENT", "KEYWORD", "PUNCT", "STRING", "EOF"
};
...
static const char *nd_name(int kind)
{
    ...
    case ND_DOWHILE:   return "DOWHILE";
...
static void dump_node(Node *n, int depth)
{
    ...
    if (n->kind == ND_CASE)
        printf(" %d", n->case_val);
    if (n->kind == ND_VAR || n->kind == ND_CALL ||
        n->kind == ND_FUNC || n->kind == ND_MEMBER)
        printf(" '%s'", n->name);
    if (n->kind == ND_VAR_DECL && n->var)
        printf(" '%s'", n->var->name);
    if (n->kind == ND_STR) {
        printf(" ");
    ...
    if (n->ty)
        printf("  :: %s", type_name(n->ty));
    if (n->kind == ND_VAR && n->var && !n->var->is_global)
        printf("  [fp-%d]", n->var->offset);
    printf("\n");
