...
void replace_and_print(char *line) {
    ...
    for (int i = 0; line[i];) {
        ...
        for (int m = 0; m < macro_count; m++) {
            ...
            if (strncmp(&line[i], macros[m].name, nlen) == 0 && (i == 0 || !isalnum((unsigned char)line[i - 1])) &&
                !isalnum((unsigned char)line[i + nlen])) {
                for (int j = 0; macros[m].value[j]; j++) out[oi++] = macros[m].value[j];
                i += nlen;
                replaced = 1;
                break;
            }
        }
        if (!replaced) out[oi++] = line[i++];
    }
    out[oi] = '\0';
    printf("%s", out);
}
...
int main(void) {
    ...
    while (fgets(line, sizeof(line), stdin)) {
        if (strncmp(line, "#define ", 8) == 0) {
            ...
            while (*p == ' ') p++;
            int i = 0;
            while (*p && !isspace((unsigned char)*p)) macros[macro_count].name[i++] = *p++;
