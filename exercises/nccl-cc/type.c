...
int ptr_elem_size(Type *ty) {
    if (!ty) return 4;
    if (ty->kind == TY_PTR && ty->base) return ty->base->size;
    if (ty->kind == TY_ARRAY && ty->base) return ty->base->size;
    return 4;
}
...
int types_compatible(Type *a, Type *b) {
    ...
    if ((a->kind == TY_INT || a->kind == TY_CHAR) && (b->kind == TY_INT || b->kind == TY_CHAR)) return 1;
    /* pointer types: any pointer assigns to any pointer */
    if (is_pointer_like(a) && is_pointer_like(b)) return 1;
    /* int to pointer (e.g. NULL) */
    if ((a->kind == TY_INT && is_pointer_like(b)) || (is_pointer_like(a) && b->kind == TY_INT)) return 1;
    return 0;
}
...
Type *decay_array(Type *ty) {
    if (!ty) return ty;
    if (ty->kind == TY_ARRAY) return ptr_to(ty->base);
    return ty;
}
...
Type *add_result_type(Type *lhs, Type *rhs) {
    ...
    if (is_pointer_like(lhs) && (rhs->kind == TY_INT || rhs->kind == TY_CHAR)) return lhs;
    /* int + ptr → ptr */
    if ((lhs->kind == TY_INT || lhs->kind == TY_CHAR) && is_pointer_like(rhs)) return rhs;
    /* int + int → int */
    return ty_int();
}
...
Type *sub_result_type(Type *lhs, Type *rhs) {
    ...
    if (is_pointer_like(lhs) && is_pointer_like(rhs)) return ty_int();
    return ty_int();
}
...
Type *promote_type(Type *ty) {
    if (!ty) return ty_int();
    if (ty->kind == TY_CHAR) return ty_int();
    return ty;
}
