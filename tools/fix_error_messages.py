...
REPLACEMENTS = {
    ...
    # 21
    '#error TODO: while(1) { getchar, buf 累积，yy_nxt 查表，state<0 时处理 token(yy_accept+ 打印+ungetc) }':
    ...
    '#error TODO: while find_istr(p,"href=") → 跳过 href= → 检测引号 → 提取 URL → printf("[%d] %s\\n")':
    ...
    '#error TODO: while find_istr(p,"href=") → p+=5 → 跳空格 → 检测引号 → 提取 URL → printf("[%d] %s\\n")':
        '#error TODO: Extract URLs from href= attributes in HTML. Run "clings hint" for help.',
